...
To highlight the benefit of using the BFCA approach, we conducted a comparative analysis with the 2SFCA method. The latter method does not normalize the impedance to allocate population and level of service proportionally.
...
A unique property of the balanced FCA method is that data can be reaggregated while preserving the total population in the core service area and the supply at each docking station. This avoids demand and supply inflation, and also enables us to present findings in a way that is perhaps more intuitive to interpret. Therefore, we reaggregated population and accessibility from the 50-by-50 m population cells to the dissemination area. We then can compared accessibility to median total household income statistics from the 2016 Canadian Census.
...
It does not have the property of preserving these quantities. This produces a There is an inflation/deflection effect
