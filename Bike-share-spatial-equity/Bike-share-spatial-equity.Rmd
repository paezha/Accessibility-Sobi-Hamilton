...
results_6min_conventional <- ttm_walk %>%
  dplyr::filter(hub_type == "Conventional" &
                  hub_status == "Active") %>%
  b2sfca(threshold = 6)
```
...
results_6min_all <- ttm_walk %>%
  dplyr::filter(hub_status == "Active") %>%
  b2sfca(threshold = 6)
```

```{r, med-sum-conventional, echo=FALSE}
sum(results_6min_conventional$los$los)
sum(results_6min_conventional$accessibility$accessibility)
```
...
Coming back to the question of accessibility by income:
```{r}
# Find the income quintiles of the population served by SoBi
iq <- hamilton_da_2016 %>% 
  dplyr::filter(sobi_service == "Yes") %>%
...
inc_stats_All_10 <- agg_accessibility_income %>%
  st_drop_geometry() %>%
  dplyr::filter(system == "All stations", threshold == "10 min") %>%
...
inc_stats_Conv_10 <- agg_accessibility_income %>%
  st_drop_geometry() %>%
  dplyr::filter(system == "Conventional", threshold == "10 min") %>%
  group_by(income_quintile) %>%
  summarize(accessibility = sum(accessibility),
