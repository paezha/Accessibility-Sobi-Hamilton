...
```{r, include=FALSE}
equity_stations <- data.frame(longitude = c(	
-79.84364993846138, -79.8362839215856, -79.83144521746212, -79.82217214949738, -79.8160211746482, -79.8192304372373, -79.82500255113169, -79.8268599805827, -79.83888834759689, -79.82740849222654, -79.82342541216097, -79.84240606419081), latitude = c(43.265340158000654, 43.25632467284136, 43.25528943845642, 43.25291050074978, 43.25147804403345, 43.24444659758296, 43.2421627336722, 43.24634382671116, 43.249432741404554, 43.251126444296524, 43.24892849896579, 43.25831920064561))
```
...
```{r, echo=FALSE, fig.align = 'center', out.width = "65%", fig.cap = "The core service area of SoBi Hamilton is outlined in blue. Dissemination areas in the Hamilton Census Metropolitan Area (CMA) are outlined in grey."}
ggplot() + 
...
```{r, include=FALSE}
eri_hubs <- sobi_hubs %>% 
             filter(OBJECTID>=117) %>% filter(OBJECTID<=128)
```

```{r figure-1, echo=FALSE, fig.align = 'center', out.width = "65%", fig.cap = "The spatial distribution of PBSP stations in Hamilton, Ontario. The service area of the PBSP is outlined in blue, and dissemination areas within the Hamilton CMA that can reach a SoBi hub within 30 minutes of walking are outlined in grey."}
ggplot() + 
  geom_sf(fill = "antiquewhite1") +
  geom_sf(data = hamilton_da_sobi, aes(color = "gray")) +
  geom_sf(data = sobi_service, fill = NA, aes(color = "blue")) + 
  geom_sf(data = sobi_hubs_original, size = 1, aes(color = "red")) +
  geom_sf(data = eri_hubs, size = 1, aes(color = "green"))+
  scale_color_identity(name = "Station Type",
                       breaks = c("red", "green"),
                       labels = c("Original Station", "Equity Station"),
                       guide = "legend")
```
...
ggplot() + 
  geom_sf(fill = "antiquewhite1") +
  geom_sf(data = population_sobi, aes(color = population)) +
  scale_color_distiller(palette = "YlOrRd", direction = 1) +
  geom_sf(data = hamilton_da_sobi, fill = NA, color = "lightgray") + 
  geom_sf(data = sobi_hubs_original, size = 1.5, shape = 21, fill = "red") +
  geom_sf(data = eri_hubs, size = 1.5, shape = 21, fill = "green") +
  geom_sf(data = sobi_service, fill = NA, colour = "blue")
```
...
### Threshold of 3 Minutes Walk

```{r, echo=FALSE}
#minimum threshold for accessibility calculations
dist_threshold <- 3
```
...
ttm_walk_o_min <- ttm_walk_o %>%
  mutate(impedance_binary = ifelse(travel_time <= dist_threshold, 1, 0))
```

```{r echo=FALSE}
sum_b1_min <- ttm_walk_min %>%
  group_by(UID) %>%
  summarize(sum_b1_min = sum(impedance_binary),
            .groups = "drop")

sum_b1_o_min <- ttm_walk_o_min %>%
  group_by(UID) %>%
  summarize(sum_b1_o_min = sum(impedance_binary),
            .groups = "drop")
```

```{r echo=FALSE}
sum_b2_min <- ttm_walk_min %>%
  group_by(hub) %>%
  summarize(sum_b2_min = sum(impedance_binary))

sum_b2_o_min <- ttm_walk_o_min %>%
  group_by(hub) %>%
  summarize(sum_b2_o_min = sum(impedance_binary))
```

```{r echo=FALSE}
ttm_walk_min <- ttm_walk_min %>%
  left_join(sum_b1_min, by = "UID") %>%
  left_join(sum_b2_min, by = "hub")

ttm_walk_o_min <- ttm_walk_o_min %>%
  left_join(sum_b1_o_min, by = "UID") %>%
  left_join(sum_b2_o_min, by = "hub")
```

```{r echo=FALSE}
ttm_walk_min <- ttm_walk_min %>%
  dplyr::filter(sum_b1_min > 0)

ttm_walk_o_min <- ttm_walk_o_min %>%
  dplyr::filter(sum_b1_o_min > 0)
```

```{r echo=FALSE}
ttm_walk_min <- ttm_walk_min %>% 
  dplyr::mutate(balanced_impedance_1 = impedance_binary/sum_b1_min, 
         balanced_impedance_2 = impedance_binary/sum_b2_min)

ttm_walk_o_min <- ttm_walk_o_min %>% 
  dplyr::mutate(balanced_impedance_1 = impedance_binary/sum_b1_o_min, 
         balanced_impedance_2 = impedance_binary/sum_b2_o_min)
```
...
```{r, include=FALSE}
sobi_los_min <- ttm_walk_min %>%
  dplyr::filter(impedance_binary > 0) %>% # filter impedance == 0 to avoid divisions by zero
  group_by(hub) %>%
  summarize(sobi_los_min = first(racks) / sum((population * balanced_impedance_1)),
            .groups = "drop")

sobi_los_o_min <- ttm_walk_o_min %>%
  dplyr::filter(impedance_binary > 0) %>% # filter impedance == 0 to avoid divisions by zero
  group_by(hub) %>%
  summarize(sobi_los_o_min = first(racks) / sum((population * balanced_impedance_1)),
            .groups = "drop")
```

```{r, include=FALSE}
ttm_walk_min <- ttm_walk_min %>%
  left_join(sobi_los_min, by = "hub")

ttm_walk_o_min <- ttm_walk_o_min %>%
  left_join(sobi_los_o_min, by = "hub")
```

```{r, include=FALSE}
sobi_accessibility_min <- ttm_walk_min %>%
  group_by(UID) %>%
  summarize(accessibility_min = sum(sobi_los_min * balanced_impedance_2),
            .groups = "drop")

sobi_accessibility_o_min <- ttm_walk_o_min %>%
  group_by(UID) %>%
  summarize(accessibility_o_min = sum(sobi_los_o_min * balanced_impedance_2),
            .groups = "drop")
```

```{r, echo=FALSE}
#current system with equity stations
sobi_accessibility_min %>% 
  pull(accessibility_min) %>% 
  sum()
...
```{r, echo=FALSE}
#original system without equity stations
sobi_accessibility_o_min %>% 
  pull(accessibility_o_min) %>% 
  sum()
...
population_sobi <- population_sobi %>%
  left_join(sobi_accessibility_min,
            by = "UID")

population_sobi_original <- population_sobi_original %>%
  left_join(sobi_accessibility_o_min,
            by = "UID")
```
...
```{r, echo=FALSE}
#accessibility plot for original system without equity stations at 3 mins walk
ggplot(data = population_sobi_original) +
  geom_sf(aes(color = accessibility_o_min)) +
  geom_sf(data = sobi_service,
          fill = NA) +
  scale_color_distiller(palette = "YlOrRd", 
                        direction = 1)
