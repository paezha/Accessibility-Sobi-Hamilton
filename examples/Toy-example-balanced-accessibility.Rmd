...
The interpolated population at unit 15 can reach eight SoBi hubs, the population at unit 16 can reach six SoBi hubs, the population at unit 30 can reach zero hubs clinics, etc. [**Note for Dr. Paez: why do the unit IDs start at 15 and not 1?** ***Note for Elise: Some population units are outside of the service area and/or cannot reach a single sobi hub by walking in 30 min or less, so they disappeared from the travel time table when it was create; see above***].

Sanity check: how many stations are within 24 or less from population UID 15?
```{r}
ttm_walk %>%
...
For instance, an analyst interested in equity analysis could allocate the total level of service uniformly to every population unit. In other words, the total level of service (which equals the sum of accessibility over the system) can be divided by the number of population units in the system to return the Average Local Population Unit PPR. The resulting mean value, call it $L_i^e$ then would be assigned to the population units as their "equitable" share of the total level of service in the system. Next, the equitative distribution of the level of service in each population unit is substracted from the estimated accessibility to arrive at a disparity index. When the difference between these two quantities is positive, this would indicate that accessibility at that location exceeds its equitable share of level of service. On the other hand, when the difference is negative, accessibility at the location is below its equitable share of the level of service.
