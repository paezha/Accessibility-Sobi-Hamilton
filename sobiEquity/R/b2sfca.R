...
#'
#' @param data A data frame with a column of origin IDs, a column of destination IDs, travel times, type of destinations, population at the origin, and supply at the destination.
...
b2sfca <- function(ttm = ttm, threshold = threshold){
  ...
  ttm <- ttm %>%
    mutate(impedance_binary = ifelse(travel_time <= threshold, 1, 0))
  ...
  los <- ttm %>%
    group_by(hub) %>%
    summarize(los = first(racks) / sum((population * balanced_impedance_1)),
              .groups = "drop")

  # Join the level of service to the table:
  ttm <- ttm %>%
    left_join(los, by = "hub")
