#' Compute conventional floating catchment area accessibility
#'
...
c2sfca <- function(ttm = ttm, threshold = threshold){
  ...
  # **Important:** notice that some values of `impedance_binary` are zeros! This means that some population units cannot reach a SoBi hub in at most the threshold. To avoid issues we will remove any population units that reach zero hubs:
  ttm <- ttm %>%
    dplyr::filter(impedance_binary > 0)

  # Calculate balanced impedance values:
  ttm <- ttm %>%
    dplyr::mutate(impedance_1 = impedance_binary,
                  impedance_2 = impedance_binary)

  # The level of service of each hub is the number of bicycle racks at the hub, divided by the population that they serve:
  los <- ttm %>%
    dplyr::group_by(hub) %>%
    dplyr::summarize(los = dplyr::first(racks) / sum((population * impedance_1)),
              .groups = "drop")
  ...
  accessibility <- ttm %>%
    dplyr::group_by(UID) %>%
    dplyr::summarize(accessibility = sum(los * impedance_2),
              .groups = "drop")
